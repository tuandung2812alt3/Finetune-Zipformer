...
import torch
from torch import Tensor, logaddexp
# from torch.cuda.amp import autocast as torch_autocast  # for SwooshRFunction
...
class SwooshL(torch.nn.Module):
    def forward(self, x: Tensor) -> Tensor:
        ...
        swoosh_l(x) = log(1 + exp(x - 4)) - 0.08 * x - 0.035
        """
        # Single implementation works for eager/JIT/ONNX
...
class SwooshR(torch.nn.Module):
    def forward(self, x: Tensor) -> Tensor:
        """Return Swoosh-R activation."""
        # For JIT / tracing / ONNX, use plain analytic form
        if torch.jit.is_scripting() or torch.jit.is_tracing():
            zero = torch.zeros_like(x)
            return logaddexp(zero, x - 1.0) - 0.08 * x - 0.313261687

        # For normal training/inference, use custom autograd function
        return SwooshRFunction.apply(x)
...
def swoosh_l_forward(x: Tensor) -> Tensor:
    """
    Swoosh-L forward:
        f(x) = log(1 + exp(x - 4)) - 0.08 * x - 0.035
    """
    zero = torch.zeros_like(x)
...
def swoosh_l_forward_and_deriv(x: Tensor):
    """
    Return (f(x), f'(x)) for Swoosh-L.
    f'(x) = sigmoid(x - 4) - 0.08
    """
...
def swoosh_r_forward(x: Tensor) -> Tensor:
    """
    Swoosh-R forward:
        f(x) = log(1 + exp(x - 1)) - 0.08 * x - 0.313261687
    """
    zero = torch.zeros_like(x)
...
def swoosh_r_forward_and_deriv(x: Tensor):
    """
    Return (f(x), f'(x)) for Swoosh-R.
    f'(x) = sigmoid(x - 1) - 0.08
    """
...
# -------------- Fused Activation + Dropout + Linear -------------- #
...
class ActivationDropoutAndLinearFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        # ----- activation forward (no k2) -----
        forward_activation_dict = {
            "SwooshL": swoosh_l_forward,
            "SwooshR": swoosh_r_forward,
        }
        # KeyError if activation not found → good, it should be caught by user
        activation_func = forward_activation_dict[activation]

        x_act = activation_func(x)

        if dropout_mask is not None:
            x_act = x_act * dropout_mask

        # linear: (..., in_features) -> (..., out_features)
        out = torch.nn.functional.linear(x_act, weight, bias)
        return out

    @staticmethod
    @custom_bwd
    def backward(ctx, ans_grad: Tensor):
        x, weight, bias, dropout_mask = ctx.saved_tensors

        # ----- activation forward+deriv (no k2) -----
        forward_and_deriv_activation_dict = {
            "SwooshL": swoosh_l_forward_and_deriv,
            "SwooshR": swoosh_r_forward_and_deriv,
        }
        func = forward_and_deriv_activation_dict[ctx.activation]

        # y: activated x; func_deriv: d f / d x
        y, func_deriv = func(x)
        ...
        # y: (..., in_channels), ans_grad: (..., out_channels)
        out_channels, in_channels = weight.shape
        g = ans_grad.reshape(-1, out_channels)                  # [N, F_out]
        y_flat = y.reshape(-1, in_channels)                     # [N, F_in]

        # dL/dW
        weight_deriv = torch.matmul(g.t(), y_flat)              # [F_out, F_in]

        # dL/dy
        y_deriv = torch.matmul(ans_grad, weight)                # same shape as y

        # dL/db
        bias_deriv = None if bias is None else g.sum(dim=0)     # [F_out]
        ...
        # gradients must match forward arguments:
        # (x, weight, bias, activation, dropout_p, dropout_shared_dim)
        return x_deriv, weight_deriv, bias_deriv, None, None, None
