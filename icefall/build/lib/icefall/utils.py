...
def concat(
    ...
    """
    Prepend or append a value to each sublist (1D Tensor) in `ragged`.

    Args:
      ragged:
        List of 1D tensors (variable length).
      value:
        The value to prepend or append.
      direction:
        "left"  -> prepend value
    ...
    Returns:
      New list of 1D tensors with the value added.
    """
    ...
    for seq in ragged:
        # keep dtype/device consistent with original sequence
        pad = torch.full(
            (1,),
            fill_value=value,
            dtype=seq.dtype,
        ...
        if direction == "left":
            new_seq = torch.cat([pad, seq], dim=0)
        else:  # "right"
...
def add_sos(ragged: List[Tensor], sos_id: int) -> List[Tensor]:
    """
    Add SOS to each sublist.
    ...
    Returns:
      New list of 1D tensors where each sublist starts with SOS.
    """
    return concat(ragged, sos_id, direction="left")


def add_eos(ragged: List[Tensor], eos_id: int) -> List[Tensor]:
    """
    Add EOS to each sublist.
    ...
    Returns:
      New list of 1D tensors where each sublist ends with EOS.
    """
    return concat(ragged, eos_id, direction="right")
